  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 mb-4 space-y-3 border border-gray-200">
      ...
      {headers.map(({ key, label }) => {
        if (key === "TicketID") return null; // Already displayed at top
              ...
              <thead className="bg-orange-300 sticky top-0 z-30">
                <tr>
                  {headers.map(({ label, key }) => (
                    <th
                      key={label}
                      className={`px-6 py-3 text-left font-bold text-black text-lg whitespace-nowrap ${key === "TicketID" ? "sticky left-0 z-40 bg-orange-300" : ""
                        }`}
                      title={label}
                    >
                      {label.substring(0, 20)}
